...
function error(msg, service, path, custom) {
  ...
  if (custom && service === "lint") {
    JSON.parse(msg)[0].warnings.forEach(value => {
      console.error(`${value.text} on line ${value.line}`);
    });
  } else if (custom && service === "compile") {
    console.error(`${infoMsg("Info:")} ${msg.message}. ${infoMsg("Status:")} ${msg.status}`);
  } else {
    console.error(msg);
  }
